/*
 *  Modifies the original OpenMD utils/ProgressBar.cpp to make it compilable to EMCC.
 *  The modified codes are wrapped in ifdef (__EMSCRIPTEN__) blocks.
 */
...
#ifdef __EMSCRIPTEN__  // in JS environment, need other method to report progress
	EM_ASM_({
		if (typeof importScripts === 'function')  // in web worker
		{
			if (typeof(postMessage) === 'function')
			{
				postMessage({'msgType': 'progress', 'percentage': $0, 'remainingTime': $1});
			}
		}
	}, percent, (current_ - start_) * (100.0/percent));
#else
          cout.width(3); 
